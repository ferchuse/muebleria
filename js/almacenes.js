...
$( document ).ready(function() {
	...
	function cargarTabla(filtros){
		...
		$("#tbody").load("control/get_tabla_almacenes.php", 
			filtros,
			function(response){
				...
				$(".editar_fila").click(function(){
					...
					data = {"table": $("#form_insert").data("table_name") , "id_field" : $("#form_insert").data("id_field"),"id_value" : id_value};
					url = "control/get_row_json.php";
					loader = boton.find(".fa")
					loader.toggleClass("fa-pencil fa-spinner fa-spin");
					console.log("update");
					$("#action").val( "update");
					$form.find(".modal-title").html("Editar Artículo" );
	...
	$(".editar_fila").click(function(){
		...
		boton= $(this);	//asignamos el buton presionado a una variable
		id_value=  boton.data("id_value"); //obtenemos el id del registro guardado en el atributo data
		data = {"table": "medicos" , "id_field" : "id_medico","id_value" : id_value};
		url = "control/get_row_json.php";
		loader = boton.find(".fa")
		loader.toggleClass("fa-pencil fa-spinner fa-spin");
		boton.attr("disabled", true);
	...
	$(".borrar_fila").click(function(){
		...
		alertify.confirm("¿Está seguro que desea borrar este Médico?", borrarFila).setHeader("Confirmar Borrado");
		
		function borrarFila(){
			data = {"table": "medicos" , "id_field" : "id_medico","id_value" : boton.data("id_value")};
